...
// It uses the `infracost_usage` struct tags to populate data into the EnSubscriptionSlack.
func (r *EnSubscriptionSlack) PopulateUsage(u *schema.UsageData) {
	resources.PopulateArgsWithUsage(r, u)
}

// BuildResource builds a schema.Resource from a valid EnSubscriptionSlack struct.
// This method is called after the resource is initialised by an IaC provider.
// See providers folder for more information.
func (r *EnSubscriptionSlack) BuildResource() *schema.Resource {
	costComponents := []*schema.CostComponent{
		EnSubscriptionSlackOutboundHTTPMessagesCostComponent(r),
	}

	return &schema.Resource{
		Name:           r.Address,
		UsageSchema:    EnSubscriptionSlackUsageSchema,
		CostComponents: costComponents,
	}
}

func EnSubscriptionSlackOutboundHTTPMessagesCostComponent(r *EnSubscriptionSlack) *schema.CostComponent {
	...
	if r.Plan == "standard" {
		...
		if r.EnSubscriptionSlack_OutboundHTTPMessages != nil {
			quantity = decimalPtr(decimal.NewFromInt(*r.EnSubscriptionSlack_OutboundHTTPMessages))
		}
